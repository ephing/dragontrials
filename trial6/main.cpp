...
static void usageAndDie(){
	...
	<< " [-m <ASMFile>]: Output MIPS assembly to <ASMFile>\n"
	;
	std::cout << std::flush;
	std::cerr << std::flush;
	exit(1);
...
static int writeMIPS(cshanty::IRProgram* prog, const char* outPath) {
	if (outPath == nullptr) {
		throw new InternalError("Null codegen file given");
	}
	if (strcmp(outPath, "--") == 0) {
		prog->toMIPS(std::cout);
	} else {
		std::ofstream outStream(outPath);
		prog->toMIPS(outStream);
		outStream.close();
	}
	return 0;
}
...
int main( const int argc, const char **argv )
{
	...
	const char * asmFilex64 = NULL;
	const char * asmFileMIPS = NULL;
	...
	try {
		...
		if (asmFilex64 != nullptr){
			if (prog == nullptr) prog = do3AC(inFile);
			if (prog == nullptr){ return 1; }
			writeX64(prog, asmFilex64);
		}
		if (asmFileMIPS != nullptr){
			if (prog == nullptr) prog = do3AC(inFile);
			if (prog == nullptr) return 1;
			writeMIPS(prog, asmFileMIPS);
		}
