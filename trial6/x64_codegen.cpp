...
void BinOpQuad::codegenX64(std::ostream& out){
	src1->genLoadValX64(out,A);
	out << "\t";
	src2->genLoadValX64(out,B);
	switch (opr) {
	case BinOp::ADD64:
		out << "\taddq " << src1->getReg(B) << ", " << src2->getReg(A) << "\n\t";
		break;
	case BinOp::SUB64:
		out << "\tsubq " << src1->getReg(B) << ", " << src2->getReg(A) << "\n\t";
		break;
	case BinOp::DIV64:
	...
	dst->genStoreValX64(out,A);
}

void UnaryOpQuad::codegenX64(std::ostream& out){
	src->genLoadValX64(out, A);
	switch (op) {
	case UnaryOp::NEG64: 
...
void EnterQuad::codegenX64(std::ostream& out){
	...
	out << "\tmovq %rsp, %rbp\n";
	out << "\taddq $16, %rbp\n";
	out << "\tsubq $" << this->myProc->arSizeX64() << ", %rsp\n\t # ^Function Header^\n";
}

void LeaveQuad::codegenX64(std::ostream& out){
	out << "\n\taddq $" << this->myProc->arSizeX64() << ", %rsp\n";
	out << "\tpopq %rbp\n";
	out << "\tretq\n";
}
...
void SetArgQuad::codegenX64(std::ostream& out){
	...
	if (index < 7) {
		...
		else getSrc()->genLoadValX64(out,r);
	} else {
		if (type->isRecord()) {
			out << "leaq " << getSrc()->getMemoryLoc() << ", %rax # get addr of record arg\n\t";
		} else {
			getSrc()->genLoadValX64(out,A);
			out << "\t";
		}
...
void GetArgQuad::codegenX64(std::ostream& out){
	...
	if (!isRecord()) {
		if (index < 7) getDst()->genStoreValX64(out,r);
		else {
...
void GetRetQuad::codegenX64(std::ostream& out){
	opd->genStoreValX64(out,A);
}

void IndexQuad::codegenX64(std::ostream& out){
	src->genLoadAddrX64(out,A);
	if (off->valString() != "0")
		out << "\taddq $" << off->valString() << ", %rax # Index, add addr offset\n\t";
	dst->genStoreAddrX64(out,A);
}

void SymOpd::genLoadValX64(std::ostream& out, Register reg){
	out << getMovOp() << getMemoryLoc() << ", " << getReg(reg) << "\n";
}

void SymOpd::genStoreValX64(std::ostream& out, Register reg){
	out << getMovOp() << getReg(reg) << ", " << this->getMemoryLoc() << "\n";
}

void SymOpd::genLoadAddrX64(std::ostream& out, Register reg) {
	out << "leaq " << getMemoryLoc() << ", " << RegUtils::reg64(reg) << "\n";
}

void AuxOpd::genLoadValX64(std::ostream& out, Register reg){
	out << getMovOp() << this->getMemoryLoc() << ", " << getReg(reg) << "\n";
}

void AuxOpd::genStoreValX64(std::ostream& out, Register reg){
	out << getMovOp() << getReg(reg) << ", " << getMemoryLoc() << "\n";
}

void AuxOpd::genLoadAddrX64(std::ostream& out, Register reg){
	TODO(Implement me)
}


void AddrOpd::genStoreValX64(std::ostream& out, Register reg){
...
void AddrOpd::genLoadValX64(std::ostream& out, Register reg){
	//store address in rbx
	out << getMovOp() << getMemoryLoc() << ", " << getReg(B) << "\n\t";
...
void AddrOpd::genStoreAddrX64(std::ostream& out, Register reg){
	out << getMovOp() << getReg(reg) << ", " << getMemoryLoc() << "\n";
}

void AddrOpd::genLoadAddrX64(std::ostream & out, Register reg){
	out << getMovOp() << getMemoryLoc() << ", " << getReg(reg) << "\n";
}

void LitOpd::genLoadValX64(std::ostream & out, Register reg){
