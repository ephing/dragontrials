...
private:
	...
	TypeAnalysis(){
		hasError = false;
		currentFnType = nullptr;
	}
	~TypeAnalysis() {
		//for (auto i : nodeToType) delete i.second;
	}

public:
	static bool build(ProgramNode * astRoot);
	...
	void errRetNoFunc(Position* pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to return outside of function");
	}
